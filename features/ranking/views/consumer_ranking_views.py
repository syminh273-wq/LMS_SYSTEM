...
class MeView(_BaseConsumerRankingView):
    """
    Get the current student's XP/level/streak summary.
...
class MeTransactionsView(_BaseConsumerRankingView):
    """
    List the current student's full XP history (unpaginated; the FE paginates).
    @param event_type: filter by event_type, e.g. quiz_passed (optional)
    @param classroom_id: filter to one classroom (optional)
    @param limit: safety cap, default 500, max 2000
    @return: XP transactions
    """
...
class MeAchievementsView(_BaseConsumerRankingView):
    """
    List the current student's achievements.
...
class MeLeaderboardView(_BaseConsumerRankingView):
    """
    Get the current student's global leaderboard rank.
    @param period: leaderboard period, default 'all'
    @return: rank, total_xp, level
    """
...
class LeaderboardView(_BaseConsumerRankingView):
    """
    Get the top-N global leaderboard.
    @param period: leaderboard period, default 'all'
