...
class ConsumerClassroomExamSubmissionViewSet(APIView):
    parser_classes = [MultiPartParser, FormParser, JSONParser]
    ...
    def get(self, request, classroom_uid, exam_uid):
        submission = self.submission_service.get_my_submission_for_classroom(
            classroom_id=classroom_uid,
            exam_id=exam_uid,
            student_id=request.user.uid,
        )
        return Response(serialize_exam_submission(submission) if submission else None)

    def post(self, request, classroom_uid, exam_uid):
        serializer = ExamSubmissionRequestSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        submission, created = self.submission_service.submit_exam_for_classroom(
            classroom_id=classroom_uid,
            exam_id=exam_uid,
            student_id=request.user.uid,
            data=serializer.validated_data,
