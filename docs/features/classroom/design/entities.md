...
**Purpose**: Lets a consumer (student) mark a classroom as favorite and retrieve a list of favorited classrooms. Lives in the Social module.

**Table**: `classroom_favorites`

| Column | Type | Key | Description |
|---|---|---|---|
| `consumer_uid` | UUID | Partition key | Student who favorited |
| `classroom_uid` | UUID v7 | Clustering key DESC | Classroom being favorited |
| `created_at` | DateTime | — | Timestamp of favorite |

**Design notes**:
- One row per `(consumer_uid, classroom_uid)`. Idempotent add / remove.
- `consumer_uid` is the partition key so the list endpoint for a user is a single partition scan.
- Counting favorites for a classroom requires a full scan (no `count(*)` on a non-partition column in Cassandra) — acceptable for the expected scale; if needed later, add a denormalized counter on the `Classroom` row.
- Soft delete is not used: a favorited classroom that is hard-removed becomes a stale row that is filtered out at response time by `ClassroomFavoriteService.list_for_consumer`.
